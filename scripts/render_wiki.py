...
def gh_table(repos: list[dict]) -> str:
    ...
    for r in repos:
        # Home-org repos display as bare name; anything else shows owner/name
        # so external repos are obvious from context.
        ...
        lines.append(
            ...
            f"{fmt(r.get('watchers', 0))} | {fmt(r.get('open_issues', 0))} | "
            f"{fmt(r.get('team_commits', 0))} | {pushed} |"
        )
    return "\n".join(lines)
...
def build() -> None:
    ...
    # ---- summary lines ----
    # Team commits across every displayed repo (both home org and external).
    gh_team_commits_now = sum(r.get("team_commits", 0) for r in gh_repos)
    prev_gh_repos = ((prev or {}).get("github") or {}).get("repos", []) if prev else []
    gh_team_commits_prev = (
        sum(r.get("team_commits", 0) for r in prev_gh_repos) if prev_gh_repos else None
    )
    gh_team_delta = delta(gh_team_commits_now, gh_team_commits_prev).strip()
    # Team-attributed lines added/deleted across every displayed repo.
    gh_team_added = sum(r.get("team_lines_added", 0) for r in gh_repos)
    gh_team_deleted = sum(r.get("team_lines_deleted", 0) for r in gh_repos)
