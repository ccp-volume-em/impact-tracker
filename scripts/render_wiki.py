...
def build() -> None:
    ...
    quay_images = (latest.get("quay") or {}).get("images", []) or []
    parts.append("\n## Quay.io container images\n")
    if not quay_images:
    ...
    # ---- Team commits within configured external scope (aggregate only) ----
    team_now = (latest.get("team_commits") or {}).get("members", []) or []
    team_prev = ((prev or {}).get("team_commits") or {}).get("members", []) or []
    parts.append("\n## Total commits by CCP-volumeEM team members\n")
    if not team_now:
        parts.append("_No team members configured, or no external scope set._")
    else:
        total_now = sum(m.get("total_commits", 0) for m in team_now)
        total_prev = sum(m.get("total_commits", 0) for m in team_prev) if team_prev else None
        d = delta(total_now, total_prev).strip()
        parts.append(
            f"_Public commits by CCP-volumeEM team members within the configured scope. "
            f"Total: **{fmt(total_now)}**{(' ' + d) if d else ''}._\n"
        )

        # per-scope totals aggregated across all members (no per-user rows)
        all_scopes = sorted({k for m in team_now for k in (m.get("per_scope") or {})})
        if all_scopes:
            scope_totals_now = {
                s: sum((m.get("per_scope") or {}).get(s, 0) for m in team_now)
                for s in all_scopes
            }
            scope_totals_prev = {
                s: sum((m.get("per_scope") or {}).get(s, 0) for m in team_prev)
                for s in all_scopes
            } if team_prev else {}
            lines = [
                "| Scope | Commits | Δ vs. previous |",
                "|---|---:|---:|",
            ]
            for s in sorted(all_scopes, key=lambda k: scope_totals_now[k], reverse=True):
                d_s = delta(scope_totals_now[s], scope_totals_prev.get(s)).strip()
                lines.append(f"| `{s}` | {fmt(scope_totals_now[s])} | {d_s or '—'} |")
            parts.append("\n".join(lines))
