...
def build() -> None:
    ...
    # Now sourced from per-repo team_commits fields set by annotate_team_commits().
    team_total_now = sum(r.get("team_commits", 0) for r in gh_repos)
    prev_gh = (prev or {}).get("github", {}).get("repos", []) if prev else []
    team_total_prev = sum(r.get("team_commits", 0) for r in prev_gh) if prev_gh else None
    d_team = delta(team_total_now, team_total_prev).strip()
    parts.append("\n## Total commits by CCP-volumeEM team members\n")
    parts.append(
        f"_Across all repos in this report: **{fmt(team_total_now)}**"
        f"{(' ' + d_team) if d_team else ''}._"
    )
